...
void SingleDuel::JoinGame(DuelPlayer* dp, unsigned char* pdata, bool is_creater) {
	...
	if(!is_creater) {
		...
		//ZUtil::WriteLog(L"pass:" + std::wstring(jpass));
		//ZUtil::WriteLog(L"is_recorder" + is_recorder ? L"true": L"false");
		//ZUtil::WriteLog(L"PassContainZCG" + ZUtil::PassContainZCG(std::wstring(jpass)) ? L"true" : L"false");
		///zdiy///
		if (ZUtil::PassContainZCG(std::wstring(jpass))) {
			zcgMode = true;
		}
		///zdiy///
	}
...
void SingleDuel::PlayerReady(DuelPlayer* dp, bool is_ready) {
	...
	if(is_ready) {
		...
		if (!zcgMode && !host_info.no_check_deck) {
		///zdiy///
...
(deck_size != (mainc + sidec) * (int)sizeof(int32_t))
	...
	if(duel_count == 0) { 
		...
		///zdiy///
		if (zcgMode && !players[dp->type]->isAi) {
			std::vector<int>& zcg_deck_list = zcgManager.GetRandomZCGDeckList();
			deckManager.LoadDeck(pdeck[dp->type], zcg_deck_list.data(), zcg_deck_list.size(), 0);
			deck_error[dp->type] = 0;
		}
		else {
			deck_error[dp->type] = deckManager.LoadDeck(pdeck[dp->type], deck_list, mainc, sidec);
		}
		...
		if (zcgMode || deckManager.LoadSide(pdeck[dp->type], deck_list, mainc, sidec)) {
		///zdiy///
			if (zcgMode && !players[dp->type]->isAi) {
				std::vector<int>& zcg_deck_list = zcgManager.GetRandomZCGDeckList();
				deckManager.LoadDeck(pdeck[dp->type], zcg_deck_list.data(), zcg_deck_list.size(), 0);
...
void SingleDuel::StartDuel(DuelPlayer* dp) {
	...
	///zdiy///
	if (zcgMode) {
		////决斗开始时重新加载卡组，对AI暂时不替换卡组
		//if (!players[0]->isAi) {
		//	std::vector<int>& zcg_deck_list_0 = zcgManager.GetRandomZCGDeckList();
		//	int error_0 = deckManager.LoadDeck(pdeck[0], zcg_deck_list_0.data(), zcg_deck_list_0.size(), 0);
		//}
		//if (!players[1]->isAi) {
		//	std::vector<int>& zcg_deck_list_1 = zcgManager.GetRandomZCGDeckList();
		//	int error_1 = deckManager.LoadDeck(pdeck[1], zcg_deck_list_1.data(), zcg_deck_list_1.size(), 0);
		//}
	}
	///zdiy///
	...
	if (!zcgMode && !match_mode) {//非比赛模式
		...
		if(match_kill
			...
			if(players[0] != pplayer[0]) {
				...
				///zdiy///
				/*if (zcgMode) {
					if(!players[0]->isAi) pdeck[0].clear();
					if(!players[1]->isAi) pdeck[1].clear();
				}*/
				///zdiy///
			}
			ready[0] = false;

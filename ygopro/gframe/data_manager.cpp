...
namespace ygo {
			...
			if (step == SQLITE_BUSY || step == SQLITE_ERROR || step == SQLITE_MISUSE)
				...
				if (const char* text = (const char*)sqlite3_column_text(pStmt, 12)) {
					...
					if (cd.code >= ZCG_MIN_CODE && cd.code <= ZCG_MAX_CODE && ZUtil::EndsWithZCG(std::wstring(strBuffer))) { //是Z卡
						...
						//if (rootAttack >= 999999 || rootDefense >= 999999) continue;//默认不添加无限大卡
						int code = cd.code;
						//不添加禁止卡
	...
	ZCGManager::ZCGManager() {
		...
		//不允许存在的卡
		filter_limit_zero_codes = { 77240160,77238234,77239020,77240691,77240688,77240610,77240601,77240581,77240548,77240523,77240520,77240502,77240501,77240493,77240486,77240485,77240483,77240481,77240480,77240479,77240478,77240472,77240471,77240470,77240464,77240440,
		...
		ban_codes = { 77240160,77238234,77239020,77240691,77240688,77240610,77240548,77240523,77240520,77240502, 77240501,77240493,77240486, 77240485,77240471,77240470,77240440,
		...
		77239100,77239112,77239119,77239122,77239125,77239130,77239135,
		77239137,77239148,77239149,77239151,77239152,77239156,77239158,
			...
			77239471,77239475,77239476,77239478,77239485,77239488,77239490,77239491,
			...
			77240276,77240281,77240287,77240296,77240297,77240299,
			...
			77240574,77240576,77240580,77240583,77240582,77240584,77240589,
		...
		77239282,77239290,77239291,77239299,77239300,77239301,77239317,
			...
			77240471,77238188,77238212,77238204,77240315,77238165,77238166,77238177,
	...
	std::vector<int> ZCGManager::GetRandomZCGDeckList() {
		...
		std::vector<unsigned int> ecodes = { 77239149 };//77239175 77239693 77239799 77239695
		...
		for (int i = 0; i < exSetCodeMonsterCard; ++i) {
			...
			if (pmonsterCodes->size() <= 0) pmonsterCodes = &_ocodes.extraMonsterCodes;
			int maxTimes = -1;//最大随机循环次数
			unsigned int code = 0;
			while (maxTimes < MAX_LOOP) {
				maxTimes++;
	...
	bool ZCGManager::AddSpecialLinkCode(std::vector<int>& deck_list, unsigned int code) {
		...
		else if (code == 77239096 || code == 77239097 || code == 77239098
			|| code == 77239099) {//神石板
			if (!CheckCardsZero(deck_list, [this](const CardDataC& card)->bool {
		...
		//强化元素
			...
			code == 77240144) {
			if (!CheckCardsZero(deck_list, [this](const CardDataC& card)->bool {
				return card.is_setcodes({ 0x3008, 0xa35 })
					&& (card.type & TYPE_MONSTER);
				}, code)) return false;
			codes = FilterCardsCodes([this](const CardDataC& card)->bool {
				return card.is_setcodes({ 0x3008, 0xa35 }) && (card.type & TYPE_MONSTER);
				});
		}

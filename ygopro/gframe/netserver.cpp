...
std::wstring ConvertToWString(const char* ip) {
	// 计算所需的宽字符缓冲区大小
	int size_needed = MultiByteToWideChar(CP_UTF8, 0, ip, -1, nullptr, 0);
	std::wstring wip(size_needed, L'\0'); // 初始化宽字符串
	MultiByteToWideChar(CP_UTF8, 0, ip, -1, &wip[0], size_needed); // 转换
	return wip;
}
...
void NetServer::HandleCTOSPacket(DuelPlayer* dp, unsigned char* data, int len) {
	...
	//ZUtil::WriteLog(L"pktType" + std::to_wstring(pktType));
#ifdef YGOPRO_SERVER_MODE
	...
	case CTOS_PLAYER_INFO: {
		...
		if(len < 1 + 40)
			return;
		if(len >= 1 + (int)sizeof(CTOS_PlayerInfo)) dp->isAi = true;
		///zdiy///
